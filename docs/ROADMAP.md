...
- **Task 002: 전체 라우트 구조 및 빈 페이지 생성 - 우선순위**
  - `/dashboard` - 일반 사용자 대시보드 페이지 (빈 껍데기)
  - `/events/create` - 이벤트 생성 페이지 (빈 껍데기)
  - `/events/[id]` - 이벤트 상세 페이지 (빈 껍데기)
  - `/events/[id]/edit` - 이벤트 수정 페이지 (빈 껍데기)
  - `/events/[token]` - 공유 링크 접근 페이지 (빈 껍데기)
  - `/notifications` - 알림 페이지 (빈 껍데기)
  - `/profile` - 프로필 페이지 (빈 껍데기)
  - `/admin/dashboard` - 관리자 대시보드 (빈 껍데기)
  - `/admin/events` - 이벤트 관리 페이지 (빈 껍데기)
  - `/admin/users` - 사용자 관리 페이지 (빈 껍데기)
  - `/admin/analytics` - 통계 분석 페이지 (빈 껍데기)
  - 모든 페이지에 기본 레이아웃 및 타이틀만 포함

- **Task 003: 공통 레이아웃 및 네비게이션 골격 구현**
  - 헤더 컴포넌트 골격 (`components/layout/header.tsx`) - 로고, 네비게이션, 알림 벨, 프로필 아이콘
  - 일반 사용자용 네비게이션 메뉴 구조
  - 관리자용 사이드바 레이아웃 구조 (`components/admin/admin-sidebar.tsx`)
  - 반응형 모바일 메뉴 골격
  - 다크 모드 토글 컴포넌트 (이미 존재하므로 위치 조정)
...
- **Task 008: 일반 사용자 대시보드 UI 완성**
  - 헤더 영역 (제목, "새 이벤트 만들기" 버튼)
  - "주최한 이벤트" 섹션 - 더미 이벤트 카드 3x3 그리드 표시
  - "참여한 이벤트" 섹션 - 더미 이벤트 카드 2x3 그리드 표시
...
- **Task 017: 반응형 디자인 및 다크 모드 적용**
  - 모든 페이지 모바일/태블릿/데스크톱 반응형 검증
  - 다크 모드 색상 변수 검증 (Tailwind dark: variants)
  - 터치 인터랙션 최적화 (버튼 크기, 간격)
  - 접근성 검증 (키보드 네비게이션, ARIA 레이블)
  - 디자인 시스템 일관성 검증 (색상, 간격, 타이포그래피)
